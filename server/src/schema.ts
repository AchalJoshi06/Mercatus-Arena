...
create table if not exists event_config (
  ...
  flash_decay numeric not null default 0.9,
  circuit_pct numeric not null default 0.1,
...
export async function migrate(): Promise<void> {
  ...
  await query(
    `alter table scoring add column if not exists pnl_score numeric(5,2)`,
  );
  await query(
    `alter table scoring add column if not exists drawdown_score numeric(5,2)`,
  );
  await query(
    `alter table scoring add column if not exists risk_score numeric(5,2)`,
  );
  await query(
    `alter table scoring add column if not exists consistency_score numeric(5,2)`,
  );
  await query(
    `alter table scoring add column if not exists efficiency_score numeric(5,2)`,
  );
  await query(
    `alter table event_config add column if not exists flash_symbols text not null default ''`,
  );
  await query(
    `alter table event_config add column if not exists flash_decay numeric not null default 0.9`,
  );
  await query(
    `alter table event_config add column if not exists circuit_pct numeric not null default 0.1`,
  );
  await query(
    `alter table order_logs add column if not exists fee numeric(12,2) not null default 0`,
  );
  await query(
    `alter table market_datasets add column if not exists live_start_t bigint`,
  );
  await query(
