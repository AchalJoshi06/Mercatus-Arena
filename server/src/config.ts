...
export const config = {
  ...
  allowedOrigins: (process.env.CORS_ORIGIN ?? "http://localhost:3030")
    .split(",")
    .map((s) => s.trim())
    .filter(Boolean),
  ...
  rateLimit: {
    ...
    authPerMin: Number(process.env.RATE_LIMIT_AUTH ?? 500),
    tradePerMin: Number(process.env.RATE_LIMIT_TRADE ?? 60),
  },
  crash: {
    auto: (process.env.CRASH_AUTO ?? "1") === "1",
    windowSeconds: Number(process.env.CRASH_WINDOW_S ?? 600),
    shockMinPct: Number(process.env.CRASH_SHOCK_MIN_PCT ?? 0.02),
    shockMaxPct: Number(process.env.CRASH_SHOCK_MAX_PCT ?? 0.04),
    probNone: Number(process.env.CRASH_PROB_NONE ?? 0.03),
    probTwo: Number(process.env.CRASH_PROB_TWO ?? 0.02),
    decay: Number(process.env.CRASH_DECAY ?? 0.9),
  },
  brokerage: {
    feePerTrade: Number(process.env.FEE_PER_TRADE ?? 10),
    freeTrades: Number(process.env.FREE_TRADES ?? 100),
  },
  llm: {
