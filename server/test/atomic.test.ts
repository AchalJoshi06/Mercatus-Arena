...
describe("executeOrder atomicity", () => {
  ...
  it("charges no fee for the first 100 trades and ₹10 after", async () => {
    await seedPrice("ATOM", 100);
    const teamId = await makeTeam("fee");
    await query(
      `insert into order_logs (team_id, action, symbol, quantity, price_executed, status, fee)
       select $1, 'BUY', 'ATOM', 1, 100, 'SUCCESS', 0
       from generate_series(1, 99)`,
      [teamId],
    );
    const free = await executeOrder({
      teamId,
      action: "BUY",
      symbol: "ATOM",
      quantity: 10,
    ...
    expect(free.cashAfter).toBe(START_CAPITAL - 1000);

    const paid = await executeOrder({
      teamId,
      action: "BUY",
      symbol: "ATOM",
      quantity: 10,
    ...
    expect(paid.fee).toBe(10);
    expect(paid.cashAfter).toBe(START_CAPITAL - 1000 * 2 - 10);

    const row = await query(
      `select fee from order_logs where order_id = $1`,
      [paid.orderId],
    );
    expect(Number(row.rows[0].fee)).toBe(10);
  });

  it("deducts the ₹10 fee from sell proceeds", async () => {
    await seedPrice("ATOM", 100);
    const teamId = await makeTeam("feesell");
    await query(
      `insert into order_logs (team_id, action, symbol, quantity, price_executed, status, fee)
       select $1, 'BUY', 'ATOM', 1, 100, 'SUCCESS', 0
       from generate_series(1, 99)`,
    ...
    expect(r.cashAfter).toBe(START_CAPITAL - 10);
  });

  it("treats the fee as required funds when buying", async () => {
    await seedPrice("ATOM", 100);
    const teamId = await makeTeam("feepoor", 1000);
    await query(
      `insert into order_logs (team_id, action, symbol, quantity, price_executed, status, fee)
       select $1, 'BUY', 'ATOM', 1, 100, 'SUCCESS', 0
